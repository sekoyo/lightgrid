import { createMemo, JSX, splitProps, type ParentProps } from "solid-js";
...
export const H2 = (props: ParentProps) => (
  <h2 class={styles.h2}>{props.children}</h2>
);
export const H3 = (props: ParentProps) => (
  <h3 class={styles.h3}>{props.children}</h3>
);
export const H4 = (props: ParentProps) => (
  <h4 class={styles.h4}>{props.children}</h4>
);
export const H5 = (props: ParentProps) => (
  <h5 class={styles.h5}>{props.children}</h5>
);
export const P = (props: ParentProps) => (
  <p class={styles.p}>{props.children}</p>
);
...
export const UL = (props: ParentProps) => (
  <ul class={styles.ul}>{props.children}</ul>
);
export const OL = (props: ParentProps) => (
  <ol class={styles.ol}>{props.children}</ol>
);
export const LI = (props: ParentProps) => (
  <li class={styles.li}>{props.children}</li>
);
...
export const IFrame = (props: JSX.IframeHTMLAttributes<HTMLIFrameElement>) => {
  const { state } = useTheme();
  const [local, attrs] = splitProps(props, ["class"]);
  return (
    <div class={cls(styles.iframeContainer, local.class)}>
      <iframe
        {...attrs}
        class={styles.iframe}
        src={`${attrs.src || ""}?theme=${state.theme}${
          attrs.height ? `&height=${encodeURIComponent(attrs.height)}` : ""
        }`}
      />
    </div>
  );
...
export const MessageBox = (
  props: ParentProps<{ level: "info" | "warning" }>
) => (
